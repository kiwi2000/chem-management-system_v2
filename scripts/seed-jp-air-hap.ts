 ...
 *
 * **判定には使わない設定で作る。**答申は「該当する可能性がある物質」を並べた候補の一覧で、
 * 含有率で該非が決まる規制ではない。判定に出すとほとんどの製品で該当が並び、
 * 本当に効く規制が埋もれる。使うかどうかは区分の「判定に使う」印で切り替えられる。
 */
...
const CATEGORIES = [
  {
    ...
    order: 10,
    note:
      "中央環境審議会 第9次答申 別表1。健康リスクがある程度以上と考えられる物質を並べたもので、" +
      "含有率で該非が決まる規制ではない。既定では判定に使わない",
  },
  {
    annex: "2",
    code: "HAP-PRI",
    ...
    order: 11,
    note:
      "中央環境審議会 第9次答申 別表2。別表1のうち、特に優先的に対策に取り組むべきとされた物質。" +
      "既定では判定に使わない",
  },
] as const;
