...
#
# 復元例（--clean 付きで取っているので、いまの中身をバックアップ時点の内容に置き換える。
# 先にアプリを止める: docker compose -f compose.prod.yml --env-file .env.prod stop app）:
...
# --clean --if-exists: 復元先に表が残っていても、そのまま流し込めるようにする
$COMPOSE exec -T db pg_dump --clean --if-exists -U chem chem | gzip > "$FILE"
