...
export function normalizeNumber(raw) {
  ...
  */
  /*
    化学兵器禁止法。CHRIP は「政令別表2項第4欄の(1)」、本システムは「令別表第2項第4欄(1)」。
    **項を落としてはいけない。**どの項にも第三欄(1)・第四欄(1)がある
  */
  const cwca = /^令別表(\d+)項第(\d+)欄の\((\d+)\)$/.exec(s);
  if (cwca) s = `令別表第${cwca[1]}項第${cwca[2]}欄(${cwca[3]})`;
...
export function normalizeName(raw) {
  ...
  s = s.replace(/[。、,]/g, "");
  /*
    法令の原文は位置の数字を漢数字で書く（`一・二―ジクロルエチレン`）。
    CHRIP は算用数字（`１，２－ジクロルエチレン`）。**両側を算用数字に寄せる**。
    1字ずつ置き換える（`一〇` は 10）。`十` は化学名にまず出ないので触らない
  */
  s = s.replace(/[〇一二三四五六七八九]/g, (c) => KANJI_DIGIT[c]);
  return s || null;
}
