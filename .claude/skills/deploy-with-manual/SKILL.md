---
name: deploy-with-manual
description: 化学物質管理システム v2 を本番（Railway / chem.ca-japan.jp）へ出す手順。ユーザーマニュアルと変更履歴を更新してからデプロイする。「デプロイして」「マニュアルを更新してデプロイ」「ここまでを反映して」と言われたら参照する。
---

# マニュアルを更新してデプロイする

このシステムのマニュアルは**公開しているサーバーから配っている**。読んでいる人が見るのは
常に反映済みのものなので、**マニュアルとデプロイは必ずセットで動かす**。

## 0. いつ始めるか

**ほかの作業が進行中なら、その作業を全部終えてから始める。**途中で出すと、
できかけの変更が本番に載るか、あとから出したぶんの変更履歴だけが抜ける。
指示が「終わったらデプロイして」なら、コミットまで済ませてからこの手順に入る。

## 1. 何を出すのかを確かめる

前回のデプロイ記録は `Record this deploy in the change log` という件名のコミット。

```bash
git log --format="%H %s" | grep "Record this deploy in the change log" | head -1
```

それ以降が今回出すもの。作業ツリーが汚れていないことも見る。

**この目印は当てにしすぎない。**変更履歴を機能のコミットに含めてしまうと、印が残らず
1回ぶん多く数えてしまう（実際に起きた）。**必ず `railway deployment list` の時刻と
コミットの時刻を突き合わせて確かめる。**

```bash
railway deployment list
```

```bash
git log --format="%h %ad %s" --date=format:"%m-%d %H:%M" -8
```

いちばん上の SUCCESS の時刻より後のコミットが、今回出すもの。

## 2. 権限が増えるときは、管理者に付けてから出す

**新しい権限を足した回は、システム管理者にその権限を付ける移行を必ず一緒に出す。**
付けないと、管理者が新しい画面を開けず、しかも**権限のせいだと気づけない**。
実際に起きた（2026-09-17 の「ドキュメントを開ける・落とせる」。
自分が作った帳票さえ開けず、別の権限を付けても直らなかった）。

増えたかどうかは、前回のデプロイからの差分で見る。

```bash
git diff <前回のデプロイのコミット>..HEAD -- packages/shared/src/permissions.ts
```

`PERMISSIONS` の並びに行が増えていたら、移行を1つ足す
（`prisma/migrations/<日時>_grant_<権限>_to_admin/migration.sql`）。

```sql
INSERT INTO "user_permissions" ("user_id", "permission", "granted_at", "granted_by")
SELECT "user_id", '<新しい権限>'::"Permission", now(), NULL
FROM "user_permissions" WHERE "permission" = 'ADMIN'
ON CONFLICT DO NOTHING;
```

- **管理者とは `ADMIN` を持つ人**（`apps/web/lib/auth.ts` の `isAdminUser`）。
  **`ADMIN` はほかの権限を含まない**ので、足さないと本当に使えない
- **既にある権限には触れない。**自分で外している人がいる。外したものを勝手に戻さない
- **管理者以外には配らない。**誰に配るかは運用の判断で、こちらで決めることではない
  （前の権限の名残で配るのが妥当なときは別。2026-09-17 は「データを出力できる」を
  持つ人に配った。そういうときは、その根拠を移行のコメントに書く）
- 変更履歴にも**「管理者には自動で付けた」**と書く。ほかの人には付いていないことも伝える

## 3. マニュアル本体を直す

`apps/web/app/spec/` 以下。変更が利用者の操作に関わるなら、該当ページを直す。

- 読み手は**一般の利用者**。化学物質管理は知っているが、IT の用語は使わない
- 操作の手順を先に書く。理屈は後
- 画面に出る文言と、マニュアルの文言を食い違わせない
- 決め打ちの値を書かない（例：パスワードの桁数は設定で変わるので書かない）

## 4. 変更履歴に1件足す

`apps/web/app/spec/changes/page.tsx` の**先頭**に `<Section>` を足す（新しいものが上）。

- 読み手は**テストに協力している利用者**。開発の記録ではない
- 「前に触ったときから何が変わったか」「操作を覚え直す必要があるか」を伝える
- **不具合の修正も書く。**「どうなっていたか → いまは」の対比が伝わりやすい
- 覚え直しが要るものは `操作が変わったところ` として先に出す
- 見出しは `YYYY-MM-DD　内容`。同じ日に複数回出すなら `（2回目）` を付ける
- **ここに書き足すのはデプロイのときだけ。**作業のたびに書くと、まだ出していないものが載る
- **利用者の画面に出ないものは書かない。**スクリプト・スキル・内部の作りの直しなど。
  読み手が知っても何もできないものを並べると、本当に読むべき行が埋もれる

## 5. 保留事項を増減させる

`apps/web/app/spec/pending/page.tsx`。できあがったものは消す（変更履歴のほうへ移る）。
取り下げになったものも消す。

## 6. 検証

```bash
npm run typecheck
```

```bash
npx eslint .
```

```bash
npx prettier --check .
```

```bash
npm test
```

そのうえで、開発サーバーを立ててマニュアルのページを実際に開き、
**書いたとおりに出ているか**を見る。特に見出しの並び順と、新しい節が出ていること。

## 7. コミットしてデプロイ

**変更履歴の更新は、必ず単独のコミットにする。**件名は
`Record this deploy in the change log`。機能のコミットに混ぜると、次回この印を探せない。

```bash
railway up --detach
```

## 8. 反映を確かめる

```bash
railway status
```

`railway up` が出したデプロイIDと、`railway status` の `deployment ID` が一致し、
status が `Online` になっていれば反映済み。

- **配信ファイルのハッシュで判断しない。** CSS を触っていない変更ではハッシュが変わらず、
  「まだ出ていない」と読み違える（実際に20分待った事故がある）
- **マニュアルの中身は外から確認できない。**ログインが要るため。
  利用者に「変更履歴の先頭を見てください」と伝えて締める

## 9. GitHub に push する

デプロイが `Online` になったら、同じ内容を GitHub にも送る。
**Railway へのデプロイは git push とは別**（作業ツリーを直接送っている）ので、
これをしないと、ソースの控えがこのPCと Railway のビルドにしか無い状態が続く
（実際に2週間・379コミットぶん溜まっていた）。

```bash
git push origin main
```

```bash
git status -sb
```

`## main...origin/main` に `ahead` が付いていなければ済み。

- ブランチは main ひとつ。**force は使わない**
- push は反映の確認（8）の**後**にする。ビルドが失敗したら直してから、失敗した版を
  GitHub に残さない
- 認証の都合で push できなかったときは、そこで止めて利用者に伝える。デプロイ自体は
  済んでいるので、やり直さない

## 10. Windows Server の評価機が動いていたら、そこへも出す

社内の評価機（`chem.lan` / 192.168.1.109、Windows Server 2025）は、お客さん向けの
インストールセットで動いている。本番と版がずれたまま試されると、直したはずの不具合が
そこで再現して混乱するので、**届く状態なら同じ版へ更新する。**届かなければ飛ばして、
そのことを利用者に伝える（評価機が止まっているのは珍しくない）。

届くかどうかは ssh で見る（鍵認証。パスワードは打たない）。

```bash
ssh -i ~/.ssh/chem-eval -o BatchMode=yes -o ConnectTimeout=10 Administrator@192.168.1.109 "Get-Content C:\chem\manifest.json"
```

返ってきた `commit` が今回出した版と違えば、次の順で更新する（全部で 15 分ほど）。

1. **開発サーバーを止める**（`next build` と `.next` を共有するため。動いたまま組むと壊れる）
2. **インストールセットを組む。** PowerShell ツールから流す（Bash ツールだと `\` が化けて始まらない）。
   10 分ほどかかるのでバックグラウンドで。記録は `out\build-install-set-N.log`（N は連番）

   ```powershell
   powershell -ExecutionPolicy Bypass -File scripts\build-install-set.ps1 *> out\build-install-set-N.log
   ```

   できた `out\chem-install-set-<版>-win64.zip`（1 GB 前後）と `.sha256` を確かめる。
   `out\install-set\...\app\manifest.json` の `commit` が今回の版になっていること

3. **評価機へ送って、ハッシュを突き合わせる**

   ```bash
   scp -i ~/.ssh/chem-eval -o BatchMode=yes out/chem-install-set-0.1.0-win64.zip out/chem-install-set-0.1.0-win64.zip.sha256 Administrator@192.168.1.109:C:/
   ```

   ```bash
   ssh -i ~/.ssh/chem-eval -o BatchMode=yes Administrator@192.168.1.109 "(Get-FileHash C:\chem-install-set-0.1.0-win64.zip -Algorithm SHA256).Hash.ToLower(); Get-Content C:\chem-install-set-0.1.0-win64.zip.sha256"
   ```

4. **評価機で展開して更新スクリプトを流す**（同期で。Start-Process で切り離すと ssh の終了時に止まる。
   記録は `updateN.log` に残す。表が変わるときは自動でバックアップを取って `prisma migrate deploy` する）

   ```bash
   ssh -i ~/.ssh/chem-eval -o BatchMode=yes Administrator@192.168.1.109 "Remove-Item -Recurse -Force C:\chem-install -ErrorAction SilentlyContinue; tar -xf C:\chem-install-set-0.1.0-win64.zip -C C:\ ; Rename-Item C:\chem-install-set-0.1.0-win64 C:\chem-install; powershell -ExecutionPolicy Bypass -File C:\chem-install\scripts\update.ps1 *> C:\chem-install\updateN.log; Get-Content C:\chem\manifest.json; Get-Service chem-app,chem-caddy,postgresql-x64-16 | Format-Table Name,Status -AutoSize; Invoke-WebRequest -UseBasicParsing http://127.0.0.1:3001/api/health | Select-Object -ExpandProperty Content"
   ```

   nssm の出力は NUL まじりなので、読むときは `tr -d '\000'` を通す

5. **確かめる。** `manifest.json` の `commit` が今回の版、サービス 3 つが Running、
   `/api/health` が `{"ok":true,"db":"up"}`。更新スクリプトの http 側の応答確認は起動直後だと
   タイムアウトすることがある（https 側が通っていれば、数秒待って確かめ直す）
6. **記録を残す。** `docs/評価機更新記録_WindowsServer_<日付>.md` に届いた版・ログの要点・気づきを追記する
   （git には入れない）。開発サーバーを起こし直す

評価機に届かなかったときは、その旨と「次に動いたときに更新が要る」ことを利用者への報告に入れる。

## この環境の制約

- `railway run` / `railway ssh` は自動承認の対象外で**使えない**。本番DBは触れない。
  データの操作が要るときは、画面から実施してもらう
- ローカルDBの検証データは、`scripts/seed-sample.ts` で入れ直せる
  （`--remove` で消える。`SB-` / `MT-` / `PR-` のコードだけを対象にする）
- 開発サーバーが動いている間に `prisma generate` や `next build` を走らせると
  `.next` が壊れる。先に止める
- **本番のDBにはトンネルで繋げる。** `DATABASE_URL` は
  `postgres.railway.internal` なので直接は届かないが、次のコマンドで
  暗号化トンネルが開く（公開プロキシを有効にする必要はない）。

  ```bash
  railway connect Postgres --tunnel-only
  ```

  出力の `postgresql://...127.0.0.1:<ポート>/railway` を一時ファイルに書いて、
  `node --env-file=<一時ファイル> node_modules/tsx/dist/cli.mjs <スクリプト>` で流す
  （`node_modules/.bin/tsx` はシェルの薄皮なので `node` から直接は動かない）。
  終わったらトンネルを閉じ、一時ファイルを消す。

  **本番に書き込む前に、必ず現状を読んで確かめる。**とくに、投入スクリプトが
  消す対象（`SB-` / `MT-` / `PR-` で始まるコード）に既存データが無いこと
