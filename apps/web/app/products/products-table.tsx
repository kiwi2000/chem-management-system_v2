...
const FILTER_LAYOUT: string[][] = [
  ["code", "status", "draftFlag", "usableAsMaterial"],
  ["nameJa"],
  ["nameEn"],
  ["modelValue", "uses"],
  ["updatedAt", "note"],
...
export function ProductsTable({ modelOptions, useOptions }: Props) {
  ...
  const columns = useMemo<TableColumn<ProductListItemDto>[]>(() => {
    ...
    return [
      ...
      {
        ...boolColumn("draftFlag", m.products.draftFlag, (r) => r.draftFlag, {
          yes: m.products.draftFlag,
          no: m.products.draftDone,
        }),
        filterLabelHidden: true,
      },
      {
  ...
  async function onMarkDoneSelected(targets: ProductListItemDto[]) {
    setError(null);
