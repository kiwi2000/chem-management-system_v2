...
export interface JudgementHitDto {
   ...
   * 同じ CAS が不純物種別違いで 2 つ並ぶことがある（`type` は 0 のとき省く）
   */
  contributions: { cas: string; pct: string; type?: string }[];
  /**
...
export interface LinkSetVersionDto {
  ...
  isCurrent: boolean;
  /** 利用者が選んだものか。立っていなければ自動（基準日がいちばん新しいもの） */
  currentPinned: boolean;
  /** 基準日（YYYY-MM-DD）。そのデータが何時点のものか。新旧はコードではなくこれで決める */
  asOf: string;
}
 ...
 */
export interface ImpurityTypeDto {
  id: string;
  code: string;
  nameJa: string;
  nameEn: string | null;
...
export type PrtrSummaryDto = ListResponse<PrtrSummaryRowDto> & { summary: PrtrSummaryMeta | null };
