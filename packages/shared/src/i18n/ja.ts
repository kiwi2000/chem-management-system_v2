...
export const ja = {
  ...
  permissions: {
    ...
    DOCUMENT_VIEW_ALL: "他の人が作ったドキュメントも見られる",
  ...
  permissionHints: {
    ...
    DATA_EXPORT: "データ入出力の TSV・JSON の書き出し。持ち出しになるので必要な人だけに",
    DATA_IMPORT:
      "データ入出力画面で、当方のデータセットや自分で作った表を取り込めます。本体を書き換えるので、既定はシステム管理者だけです",
    INACTIVE_VIEW:
      "未公開のデータと、無効にした製品を一覧・詳細で見られます（作成者は権限が無くても自分のぶんを見られます）",
    DOCUMENT_DOWNLOAD:
      "他の人が作った帳票の紙面を画面で開く・印刷する、PDF・zip を落とす。無い人は一覧（名前・日時・状態）までです。画面で開ければ印刷して保存できるので、開くのも持ち出しとして扱います。自分が作ったものは、この権限が無くても開けます",
    DOCUMENT_VIEW_ALL:
      "生成済ドキュメントに全員のぶんが並び、落とす・消すこともできます。ただし組成が載ったものは「組成を見られる」、未公開の製品・物質のものは「無効・未公開のデータも見られる」が無ければ出ません",
  ...
  laws: {
    title: "法律",
    rowHint:
      "法律のコードを押すと、その法律の区分が下に出ます。区分の「>」を押すと分類が、区分のコードを押すと法文物質名の一覧へ移ります",
    /** 分類の行を引いたとき。並びは区分の画面で変える */
    classReorderElsewhere: "分類の並びは、区分のコードを押した先の画面で変えます",
  ...
  statutorySubstances: {
    ...
    casCount: "リンク数",
    /*
      絞り込みだけの欄（2026-09-18 指示）。
      結び付いている CAS・物質のほうから法文物質名を探す
    */
  ...
  impurityTypes: {
    ...
    exemptionsHint:
      "チェックを付けた規制区分では、この種別の物質を必ず非該当にします。法律の行を押すと、その法律の区分をまとめて付け外しできます",
    exclude: "非該当にする",
    ...
    builtinCode: "組み込みの種別のコードは変えられません（名前と説明は変えられます）",
    builtinDelete: "組み込みの種別は消せません",
    inUse: (n: number) =>
      `この種別は物質${n}件で使われています。先に物質の種別を付け替えてください`,
    duplicateCode: (code: string) => `コード「${code}」は既に使われています`,
    /** 判定の結果に出す印 */
    excluded: "不純物のため非該当",
    excludedHint: "不純物種別の設定で、この規制区分では非該当と決めています",
  },
  ...
  judgements: {
    ...
    /** 判定対象日（判定は必ず持つ。2026-09-22 決定）。見出しでは読み取りだけ */
    judgedAsOf: (day: string) => `判定対象日 ${day}`,
    /** 今日でない日付で判定してある。帳票にもこの判定が載るので、はっきり断る */
    notToday: (day: string) => `今日の規制ではありません（${day} 時点の規制で判定しています）`,
    stale: "データに変更があるため、再計算で結果が変わる可能性があります。",
    staleHint:
      "この判定を計算した後に、CASリンク・閾値のどちらかが変わっています。判定し直すと結果が変わる可能性があります",
    /** 判定対象日より後に、施行日・適用終了日を迎えた法文物質名（区分）がある */
    staleByDate:
      "判定対象日より後に施行日または適用終了日を迎えた法文物質名があるため、再計算で結果が変わります。",
    /** この製品1件だけを判定し直す。押すと判定対象日を尋ねる（既定は今日） */
    rejudge: "再計算",
    rejudgeHint:
      "判定対象日を選んで、この製品だけ判定し直します（結果は保存されます）。ほかの製品は変わりません",
    rejudgeAsOf: "判定対象日",
    rejudgeAsOfHint:
      "その日に効いている規制区分と法文物質名で判定し直します。既定は今日です。今日以外にすると、その日付が判定に残ります",
    rejudgeRun: "この日付で再計算",
    /** 判定は法規制バージョンごとに持つ。切り替えた直後は、その版の判定がまだ無い */
    ...
    notApplicableHint:
      "該当していない法文物質名（含有率が足りないもの・人が非該当にしたもの・施行前や適用終了のもの）も出します。含有率不足は ⚠ 付きの橙色の字、施行前・適用終了は薄い字に印付きです",
    needsReview: "要確認",
    needsReviewHint: "適用条件を確認し、必要があれば該非を変更して下さい。",
    /** 施行前・適用終了の行は該当に数えない非該当。判断の対象にしない（2026-09-22 決定） */
    cannotDecideNotInForce:
      "施行前または適用終了の法文物質名は、判定対象日には効いていないので確認・修正できません。",
    byUser: "人が判断",
    basis: "根拠",
    ...
    /**
     * 判定対象日に効いていない法文物質名・区分（2026-09-22 決定）。
     * 該当に数えない非該当で、要確認も人の判断も付かない。いつから・いつまでかを添える
     */
    notYetEffective: (from: string) => `施行前（${from} から）`,
    expired: (to: string) => `適用終了（${to} まで）`,
    notYetCategory: (from: string) => `区分が施行前（${from} から）`,
    expiredCategory: (to: string) => `区分が適用終了（${to} まで）`,
    ...
    reasonConditionalLink:
      "CASが法文物質名に該当しない物質を含みます。名称と物質を見比べてください。",
    reasonHomogeneous:
      "この規制の濃度は均質材料あたりで決まっています。製品全体では薄まるため、この判定だけでは言い切れません。均質材料を原材料として登録し、その原材料の判定を見てください",
    reasonDecisionDropped:
      "以前に人が確認・上書きしていましたが、前提（当たる法文物質名とCAS）が変わったため、その判断は当てはめていません。判断し直してください。",
    /** 当てはめなかった以前の判断の中身。「以前の判断: 非該当（森沢・2026/9/12 17:20）」 */
    droppedDecision: (verdict: string, who: string, when: string) =>
      `以前の判断: ${verdict}（${who}・${when}）`,
    /** 以前の判断が「確認しただけ」（判定は変えていない）のとき */
    droppedConfirmOnly: "確認のみ",
    // 一覧の列。狭いので短く
    listHeader: "法規制",
  ...
  docTemplates: {
    ...
    targetHint:
      "製品・物質は 1 件につき 1 枚。組織は取引先などの組織ごとに 1 枚。対象なしは、宛先や会社などの共通の項目だけで 1 枚作ります（送付状・案内状など）。製品の一覧・物質の一覧は、選んだ全部を 1 枚の表にします。規制区分は区分ごとに 1 枚（法文物質名の表を添えられます）",
    kind: "作りかた",
  ...
  docEditor: {
    ...
    blockKinds: {
      ...
      signature: "署名欄",
      image: "画像",
      repeatStart: "繰り返しの始まり",
      repeatEnd: "繰り返しの終わり",
    },
    repeatHint:
      "「繰り返しの始まり」から「繰り返しの終わり」までを 1 件分として、選んだ製品（物質）の数だけ繰り返します。この間では製品（物質）1 件ごとの差込項目と表（組成・法規制判定など）が使えます。印そのものは紙に出ません",
    orgBlock: "組織の項目",
    orgBlockHint:
    ...
    moveDown: "下へ",
    tableColumnsHint:
      "チェックで出し入れし、▲▼で並べ替えます。紙面にはこの順で出ます。数は幅の比で、空なら 1（例: 1・3・2 なら 1:3:2）。どの列も空なら中身に合わせて自動で決まります。",
    tableColumnWidth: "幅の比",
    tableCategories: "出す規制区分",
    tableCategoriesPicked: (n: number) => `${n} 件を選択中`,
    tableCategoriesHint:
      "チェックした規制区分の行だけを出します。何も付けなければ全部出します。区分を消したり増やしたりしたときは、ここも見直してください。",
    tableFilters: "出す行を絞る",
    tableFilterColumn: "列",
    tableFilterOp: "条件",
    tableFilterValue: "値",
    ...
    unsavedOnLeave:
      "保存していない変更があります。「保存」か「変更を破棄」を選んでから、一覧へ移ってください。",
    ...
    remove: "このブロックを消す",
    reorderHint: "つまみを引くと、ブロックを並べ替えられます（▲▼でも動かせます）",
    blockCollapse: "中身をたたむ",
    blockExpand: "中身を開く",
  ...
  documents: {
    ...
    batchStarted: (n: number) =>
      `${n} 件の生成を始めました。終わるまで他の画面で作業できます。できたものは下の「生成状況」と「生成済ドキュメント」に並びます`,
    jobsTitle: "生成状況",
    jobsHint: "まとめて頼んだ生成の進捗です。終わったら「まとめて開く」で 1 回で印刷できます",
    jobsNone: "まとめて頼んだ生成はまだありません",
    jobRequestedAt: "生成開始日時",
    jobProgress: "進捗",
    jobStatus: "状態",
    jobStatuses: {
      QUEUED: "待ち",
    ...
    jobOpen: "まとめて開く",
    jobDeleteRunning: "生成中の仕事は消せません。終わってから消してください",
    createdBy: "作成者",
  ...
  substanceMatrix: {
    /** カードの見出し。該当法規とインベントリ番号の2つの表をまとめた札 */
    title: "該当法規とインベントリ番号",
    region: "地域",
    /** 左の見出し列。区分の段と、値の行に出す */
    category: "規制区分",
  ...
  settings: {
    ...
    conditionalLinkMode: "CASが法文物質名に該当しない物質を含む場合",
    // どちらも判定は該当。違いは要確認の印を付けるかどうか（2026-09-12 指示の文言）
    conditionalLinkModes: {
      hit: "「要確認」を付けない",
      review: "「要確認」を付ける",
    },
    conditionalLinkModeHint:
      "法文物質名にリンクしたCASが、法文物質名で規定された物質以外も含む場合。\n例）法文物質名が「炭素数10〜13のアルキルベンゼン」で、リンクしたCASが「炭素数10〜16のアルキルベンゼン」を指しているとき、炭素数14〜16のものも含みます。\nどちらを選んでも判定は該当です。「付ける」なら判定に「要確認」と理由が、「付けない」なら「警告」と理由が出ます。",
    rejudge: "全製品の再計算",
    /** 判定対象日（2026-09-22 決定）。既定は今日 */
    rejudgeAsOf: "判定対象日",
    rejudgeAsOfHint:
      "その日に効いている規制区分と法文物質名で全製品を判定し直します。既定は今日です",
    rejudgeHint:
      "CASリンク・閾値・データソースの優先順位を変えても、判定は自動ではやり直されません（組成を保存したときだけ、その製品と親製品がやり直されます）。法規制バージョンを切り替えたときも、その版の判定はここで作ります（判定は版ごとに保存され、前の版の判定は残ります）。法規制側のデータを変えたら、ここで全部をやり直してください。製品が多いと数十分かかります。裏で進むので、この画面を閉じてもかまいません",
    ...
    appNameJa: "システム名（日本語）",
    appNameEn: "システム名（英語）",
    appNameHint: "空の場合はデフォルトのシステム名が表示されます。",
    headerIcon: "ロゴ",
    headerIconHint:
    ...
    headerHideName: "システム名を非表示（ロゴ画像が必須）",
    headerIconPositions: {
      left: "左",
      right: "右",
    },
  ...
  composition: {
    ...
    // 原材料の中身を下ろして見せる（展開）
    expand: "中身を見る",
    /** まとめて開く・閉じるボタン。1文字＋山形の印（＞ 開・∨ 閉。いまの状態の印と同じ向き）。説明はマウスを乗せたときに出す（2026-09-12 指示） */
    /** まとめて開く・閉じるボタン。**何のボタンか分かるよう、言葉を出す**（2026-09-18 指示） */
    expandAll: "すべて開く",
    expandAllHint: "まとめて開きます",
  ...
  products: {
    ...
    /** 選んだ製品だけ判定し直す。選んでいないと出ないので、修飾は付けない */
    rejudgeSelected: "再計算",
