...
export const ja = {
  ...
  settings: {
    ...
    conditionalLinkMode: "CASだけでは判定できないものの扱い",
    // どちらも判定は該当。違いは要確認の印を付けるか、警告だけにするか（2026-09-11 指示）
    conditionalLinkModes: {
      hit: "該当にして、警告だけ出す",
      review: "該当にして、要確認を付ける",
    },
    conditionalLinkModeHint:
      "CAS番号だけでは、その法文物質名に当てはまるかを判定できないものがあります。例えば法文物質名が「炭素数10〜13のアルキル基を持つ…」のとき、外部データベースはその範囲に入りそうなCASをまとめて結び付けますが、実際に炭素数が条件に合うかはCAS番号からは分かりません。そのようなCASで当たった判定は、どちらを選んでも「該当」になり、警告欄に「条件に合うかは要確認」と出ます。違いは、要確認の印を付けて人の確認を待つか、警告だけにして確定扱いにするかです。法文物質名の「適用条件」（用途や形状による除外）はこの設定とは別で、当たれば必ず要確認になります。",
    rejudge: "全製品を判定し直す",
    rejudgeHint:
