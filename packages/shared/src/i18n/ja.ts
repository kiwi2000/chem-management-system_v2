...
export const ja = {
  ...
  judgements: {
    ...
    /** 前提が変わっているときだけ出す。この製品1件だけを、いまの前提で計算し直す */
    rejudge: "再計算",
    rejudgeHint: "いまの前提で、この製品だけ判定し直します。ほかの製品は変わりません",
    ...
    /**
     * 法文物質名に適用条件が書いてあるときは、決まり文句ではなく**その条文を出す**（2026-09-20 指示）。
     * 書いていないときだけ上の決まり文句に落ちる
     */
    reasonConditionWith: (condition: string) =>
      `適用条件「${condition}」。この条件に当たらなければ非該当になります。`,
    reasonUnfilled:
