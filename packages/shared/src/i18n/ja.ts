...
export const ja = {
  ...
  settings: {
    ...
    conditionalLinkModeHint:
      "外部データベースが法文物質名に結び付けたCASには、名称の条件（炭素数・置換位置など）に合うか確かめていないものがあります。CASの指す物質が名称より広い場合（例: 炭素数10〜16のアルキルベンゼンを「炭素数10〜13」の名称に結んでいる）と、範囲に入りそうなCASをまとめて結んだだけの場合です。そのCASで当たった判定は、どちらを選んでも該当になり、警告欄に「条件に合うかは要確認」と出ます。違いは、要確認の印を付けて人の確認を待つか、警告だけにするかです。",
    rejudge: "全製品を判定し直す",
    rejudgeHint:
